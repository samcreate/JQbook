	...
	var self = { 
		...
		*/
		...
		events : {
			...
			JQBOOK_READY:"Jqbook_is_loaded_ready",
			USER_NOT_LOGGED_IN:"User_is_not_loggedin",
			CONNECTION_READY:"User_has_logged_in_and_ready_with_asked_permissions",
			...
			ON_FRIENDSINFO_FOUND:"User_notes_data_found",
			ON_FRIENDSINFO_NOT_FOUND:"User_notes_data_not_found",
		...
		*/
		...
		options : function(opts){
			...
			return $.extend({}, self.defaults, opts);	
		},
		/*
		...
		*/
		...
		init : function(opts){
		    ...
		    if($('#fb-root').size() <= 0){
				...
				//load facebook sdk
				_loadjscssfile('http://connect.facebook.net/en_US/all.js','js');
		
			try{
			    
			        self.defaults.permissions = opts.permissions != null ? opts.permissions : self.defaults.permissions;
				
				    self.defaults.permissions =  self.defaults.permissions == '' ? null : self.defaults.permissions;
    		
				    FB.init({appId: self.options(opts).appID, status: true, cookie: true, xfbml: true});
    				
				    self.loginStatus();
				
					$("#fb-root").trigger(self.events.JQBOOK_READY);
			    
			    }catch(e){
			    
			        _opts = opts;
			       
		            var t = setTimeout(function(){self.init(_opts);},1000);
		...
		loginStatus : function(p_callback){
			...
			FB.getLoginStatus(function(response){
					...
					if (response.session){
						...
						self.uid(response.session.uid);
						
						$("#fb-root").trigger(self.events.CONNECTION_READY, [ {uid:response.session.uid} ]);
				...
				if (response.session){
					...
					FB.login(function(response) {
						...
						if (response.session) {
							...
							if (response.perms || self.defaults.permissions === null) {
								...
								logged_in = true;
								self.uid(response.session.uid);
								$("#fb-root").trigger(self.events.CONNECTION_READY_PERMISSIONS_NONE);
								_callback(p_callback,self.events.CONNECTION_READY_PERMISSIONS_NONE);
							}	
							 
						} else {
							
								$("#fb-root").trigger(self.events.CONNECTION_FAILED);
		...
		*/
		userInfo : function(p_callback){
			...
			if(logged_in ==false) { self.debug(self.events.USER_NOT_LOGGED_IN); return;}
		
			FB.api('/me', function(response){
				if(!response || response.error){
					// Facebook error getting the user's info
					$("#fb-root").trigger(self.events.ON_USERINFO_NOTFOUND);
					_callback(p_callback,false);
					return(false);
				}
				
				$("#fb-root").trigger(self.events.ON_USERINFO_FOUND, [ response ]);
		...
		*/
		userWall : function(p_limit_amount,p_callback){
		  ...
		  FB.api(
			  {
				method: 'fql.query',
				query: 'SELECT actor_id, message FROM stream WHERE source_id = '+self.uid()+' limit '+p_limit_amount
			  },
			  function(response) {
				  
				  $("#fb-root").trigger(self.events.ON_WALLINFO_FOUND, [ response ]);
				  _callback(p_callback,response);
		...
		*/
		userLikes : function(p_callback){
			...
			FB.api('/'+self.uid()+'/likes', function(response){
				if(!response || response.error){
					
					$("#fb-root").trigger(self.events.ON_LIKESINFO_NOT_FOUND);
					_callback(p_callback, false);
					return(false);
				}
			
				
				$("#fb-root").trigger(self.events.ON_LIKESINFO_FOUND, [ response.data ]);
		...
		*/
		userMovies : function(p_callback){
			...
			FB.api('/'+self.uid()+'/movies', function(response){
				if(!response || response.error){
					
					$("#fb-root").trigger(self.events.ON_MOVIESINFO_NOT_FOUND);
					_callback(p_callback, false);
					return(false);
				}
		
				
				$("#fb-root").trigger(self.events.ON_MOVIESINFO_FOUND, [ response.data ]);
				_callback(p_callback,  response.data);
		...
		*/
		userNotes : function(p_callback){
			  ...
			  FB.api('/'+self.uid()+'/notes', function(response){
				if(!response || response.error){

					$("#fb-root").trigger(self.events.ON_NOTESINFO_NOT_FOUND);
					_callback(p_callback, false);
					return(false);
				}


				$("#fb-root").trigger(self.events.ON_NOTESINFO_FOUND, [ response.data ]);
				_callback(p_callback,  response.data);

			});
		},
		/*
		   Function: JQbook.userFriends
		
		   Queries facebook for the users friends
		...
		*/
		userFriends : function(p_callback){
			
			  if(logged_in ==false) { self.debug(self.events.USER_NOT_LOGGED_IN); return;}

			  FB.api('/'+self.uid()+'/friends', function(response){
				if(!response || response.error){
					
					$("#fb-root").trigger(self.events.ON_FRIENDSINFO_NOT_FOUND);
					_callback(p_callback, false);
					return(false);
				}
		
				
				$("#fb-root").trigger(self.events.ON_FRIENDSINFO_FOUND, [ response.data ]);
				_callback(p_callback,  response.data);
		...
		*/
		userGroups : function(p_callback){
			...
			FB.api('/'+self.uid()+'/groups', function(response){
				if(!response || response.error){
					
					$("#fb-root").trigger(self.events.ON_GROUPSINFO_NOT_FOUND);
					_callback(p_callback, false);
					return(false);
				}
		
			
				$("#fb-root").trigger(self.events.ON_GROUPSINFO_FOUND, [ response.data ]);
				_callback(p_callback,  response.data);
		...
		*/
		userProfileIMGS : function(p_callback){ 
			...
			FB.api(
			  {
				method: 'fql.query',
				query: 'SELECT pic, pic_square,pic_big FROM profile WHERE id = '+self.uid()
			  },
			  function(response) {
				  
				  $("#fb-root").trigger(self.events.ON_IMAGESINFO_FOUND, [ response ]);
				  _callback(p_callback,response);
			 ...
			 FB.ui(
				...
				function(response) {
					if (response && response.post_id) {
					   $("#fb-root").trigger(self.events.ON_STREAM_PUBLISH_SUCCESS, [ response ]);
						_callback(p_callback,response);
					 } else {
						
						$("#fb-root").trigger(self.events.ON_STREAM_PUBLISH_FAILED);
						_callback(p_callback,false);
					 }
				});
