		...
		return { 
			...
			*/
			userFriends : function(p_callback){
				  ...
				  FB.api('/'+JQbook.uid()+'/friends', function(response){
					if(!response || response.error){
						
						$("#fb-root").trigger(JQbook.events.ON_FRIENDSINFO_NOT_FOUND);
						_callback(p_callback, false);
						return(false);
					}
			
					
					$("#fb-root").trigger(JQbook.events.ON_FRIENDSINFO_FOUND, [ response.data ]);
					_callback(p_callback,  response.data);
